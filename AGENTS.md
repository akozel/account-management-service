...
- After every change to Rust production code or tests, run
  `cargo llvm-cov --color never --fail-under-lines 90`. It fails automatically
  when total line coverage is below `90%`; add tests until it passes. If the
  report cannot be generated, state the exact environmental blocker in the
  handoff.
